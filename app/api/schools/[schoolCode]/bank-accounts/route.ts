...
import { createBankAccountSchema } from "@/schemas/school-bank-account.schema"
import {
  createBankAccount,
  listBankAccounts,
...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const items = await listBankAccounts(access.schoolId)
...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const created = await createBankAccount(access.schoolId, session.user.id, parsed)
    return NextResponse.json({ item: created })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ message: error.issues[0]?.message ?? "Invalid input" }, { status: 422 })
    }
    if (error instanceof Error && error.message === "BANK_ACCOUNT_LIMIT_REACHED") {
      return NextResponse.json(
        { message: "Maximum 5 bank accounts per school" },
        { status: 422 },
      )
    }
    console.error("[POST bank-accounts]", error)
    return NextResponse.json({ message: "Failed to create bank account" }, { status: 500 })
  }
}
