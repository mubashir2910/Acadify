...
// react-bits ProfileCard (TS + Tailwind). Self-contained holographic/tilt card.
// Adapted for Acadify: the info-bar top line drops the hard-coded "@" prefix and
// the second line is skipped when empty, so it fits arbitrary labels (class-sec,
// Acadify ID) instead of a social handle/status.
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const shineStyle = {
    ...
    // Softened from opacity(0.5): the holographic shine sits on top of the photo,
    // so a lower opacity keeps the subject clearly visible (esp. bright photos).
    filter: 'brightness(0.66) contrast(1.33) saturate(0.33) opacity(0.22)',
    ...
    >
      ...
      <div ref={shellRef} className="relative z-[1] group">
        <section
          ...
          style={{
            // Width-driven so the card always fits its container (prevents
            // horizontal overflow on mobile that shifted the navbar / clipped the
            // right edge). Height derives from the aspect ratio.
            width: '100%',
            height: 'auto',
            maxHeight: '540px',
            aspectRatio: '0.718',
        ...
        >
          ...
          >
            ...
            {/* Avatar content — rendered in true colour (normal blend) so ANY
                uploaded photo stays visible. A soft top/bottom scrim keeps the
                name and info bar readable over bright photos. */}
            <div
            ...
            >
              {avatarUrl && (
                ...
                <img
                  ...
                  style={{
                    ...
                    borderRadius: cardRadius,
                    backfaceVisibility: 'hidden',
                    // Gentle normalisation so bright/over-exposed phone photos
                    // don't blow out, without darkening the subject.
              ...
              {/* Readability scrim: darkens the very top (behind the name) and the
                  bottom (behind the info bar) while leaving the face untouched. */}
              {avatarUrl && (
                <div
                  className="absolute inset-0"
                  style={{
                    borderRadius: cardRadius,
                    background:
                      'linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0) 26%, rgba(0,0,0,0) 60%, rgba(0,0,0,0.45) 100%)',
                    pointerEvents: 'none'
                  }}
                />
              )}
              {showUserInfo && (
                <div
                  className="absolute z-[2] flex items-center justify-between backdrop-blur-[30px] border border-black/10 pointer-events-auto"
                  style={
                    {
                      '--ui-inset': '20px',
                      '--ui-radius-bias': '6px',
                      ...
                      left: 'var(--ui-inset)',
                      right: 'var(--ui-inset)',
                      // Frosted-white bar so the black text below reads on any photo.
                ...
                >
                  <div className="flex items-center gap-3">
                    {(miniAvatarUrl || avatarUrl) && (
                      <div
                        className="rounded-full overflow-hidden border border-black/10 flex-shrink-0"
                    ...
                    <div className="flex flex-col items-start gap-1.5">
                      {handle && (
                        <div className="text-sm font-bold text-black leading-none">{handle}</div>
                      )}
                      {status && <div className="text-sm font-medium text-black/65 leading-none">{status}</div>}
                    </div>
                  </div>
                  <button
                    className="border border-black/20 bg-white/50 rounded-lg px-4 py-3 text-xs font-semibold text-black cursor-pointer backdrop-blur-[10px] transition-all duration-200 ease-out hover:border-black/40 hover:bg-white/70 hover:-translate-y-px"
