...
import { updateBankAccountSchema } from "@/schemas/school-bank-account.schema"
import {
  deleteBankAccount,
  updateBankAccount,
} from "@/services/school-bank-account.service"
...
export async function PATCH(req: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const updated = await updateBankAccount(access.schoolId, session.user.id, id, parsed)
    ...
    console.error("[PATCH bank-accounts]", error)
    return NextResponse.json({ message: "Failed to update bank account" }, { status: 500 })
  }
}
...
export async function DELETE(_req: Request, { params }: RouteParams) {
  ...
  if (limited) return limited
  try {
    await deleteBankAccount(access.schoolId, session.user.id, id)
    return NextResponse.json({ message: "Deleted" })
  } catch (error) {
    if (error instanceof Error && error.message === "BANK_ACCOUNT_NOT_FOUND") {
      return NextResponse.json({ message: "Bank account not found" }, { status: 404 })
    }
    console.error("[DELETE bank-accounts]", error)
    return NextResponse.json({ message: "Failed to delete bank account" }, { status: 500 })
  }
}
