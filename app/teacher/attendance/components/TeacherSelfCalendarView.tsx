...
export default function TeacherSelfCalendarView() {
  ...
  const fetchMonthData = useCallback(async () => {
    try {
      const [attendanceRes, calendarRes] = await Promise.all([
        fetch(`/api/teacher-attendance/monthly?month=${monthStr}`),
  ...
  for (const r of records) statusMap.set(r.date, r.status)
  ...
  const getDayInfo = (dateStr: string) => {
    ...
    if (override) return { type: override.type, reason: override.reason }
    const d = new Date(dateStr + "T00:00:00")
    const day = d.getDay()
    return { type: (day === 0 || day === 6) ? "HOLIDAY" as const : null, reason: null }
  }

  const handleDayClick = (date: Date | undefined) => {
    if (!date) return
    const ds = format(date, "yyyy-MM-dd")
    setSelectedDayStatus(statusMap.get(ds) ?? null)
    setSelectedDayDate(ds)
  }

  // Attendance modifiers
  const presentDays = records.filter((r) => r.status === "PRESENT").map((r) => new Date(r.date + "T00:00:00"))
  const absentDays  = records.filter((r) => r.status === "ABSENT").map((r) => new Date(r.date + "T00:00:00"))
  const lateDays    = records.filter((r) => r.status === "LATE").map((r) => new Date(r.date + "T00:00:00"))

  // Combined modifiers: attendance on a special day
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) holidayDates.push(d)
  }
    ...
    if (selectedDayStatus) {
      return (
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          >
            {selectedDayStatus === "PRESENT" ? "Present" : selectedDayStatus === "ABSENT" ? "Absent" : "Late"}
          </Badge>
          {isSpecialDay && (
            <>
    ...
    <Card>
      <CardContent className="p-4">
        <Calendar
          ...
          modifiers={{
            present: presentDays, absent: absentDays, late: lateDays,
            holiday: holidayDates, halfDay: halfDayDates, event: eventDates,
            presentHalfDay: presentHalfDayDates, presentEvent: presentEventDates,
            absentHalfDay: absentHalfDayDates, absentEvent: absentEventDates,
            lateHalfDay: lateHalfDayDates, lateEvent: lateEventDates,
          }}
          modifiersClassNames={{
