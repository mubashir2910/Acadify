...
// POST /api/upload/digital-id-photo — uploads the dedicated half-body ID-card photo.
// Mirrors the profile-picture route (auth, rate limit, magic-byte check) but stores
// under its own key prefix so ID photos never overwrite avatars.
export async function POST(req: Request) {
  ...
  try {
    ...
    // Unique key per upload so a changed photo never serves a stale CDN cache.
    const url = await uploadToSpaces(buffer, {
      key: `digital-id-photos/user_${session.user.id}_${Date.now()}.${ext}`,
