...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const validRows: ReturnType<typeof csvStudentRowSchema.parse>[] = []
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        const validated = csvStudentRowSchema.parse(rawRow)
    ...
    const result = await importStudents(schoolCode, validRows)
