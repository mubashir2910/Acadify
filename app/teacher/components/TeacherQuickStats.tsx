...
export default function TeacherQuickStats({ summary, isSubmitted }: TeacherQuickStatsProps) {
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <Card className={`border-0 shadow-none ${isSubmitted ? "bg-green-500/10" : "bg-amber-500/10"}`}>
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-2">
            <ClipboardCheck className={`h-4 w-4 ${isSubmitted ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`} />
            <span className="text-xs font-medium text-muted-foreground">Today&apos;s Attendance</span>
          </div>
          <p className={`text-base font-bold ${isSubmitted ? "text-green-700 dark:text-green-400" : "text-amber-700 dark:text-amber-400"}`}>
